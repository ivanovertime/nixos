...
- `home.nix` + `home/` — User environment managed by Home Manager (terminal, editor, and the Hyprland desktop modules).

## 🪟 Desktop (Hyprland)

The desktop is a [Hyprland](https://hypr.land/) Wayland session, logged in through
`greetd` + `tuigreet`. The user-space pieces live in `home/`:

| Component      | Tool            | Module                |
| -------------- | --------------- | --------------------- |
| Compositor     | Hyprland        | `home/hyprland.nix`   |
| Status bar     | Waybar          | `home/waybar.nix`     |
| App launcher   | Wofi            | `home/wofi.nix`       |
| Notifications  | Mako            | `home/mako.nix`       |
| Lock / idle    | hyprlock / hypridle | `home/hyprland.nix` |
| Theming        | Adwaita-dark + Tela Circle | `home/gtk.nix` |

**Key bindings** (mod = `SUPER`):

- `mod + Return` terminal · `mod + R` launcher · `mod + E` files
- `mod + Q` close · `mod + F` fullscreen · `mod + V` float
- `mod + 1..0` switch workspace · `mod + Shift + 1..0` move window
- `mod + Shift + L` lock · `mod + Shift + M` exit Hyprland
- `Print` region screenshot · `mod + Shift + V` clipboard history

> **☁️ Google Drive:** GNOME Online Accounts was removed with GNOME. Mount Google
> Drive with an [`rclone`](https://rclone.org/drive/) remote exposed via a
